...
function showMap(obj)
{
  ...
  if(document.getElementById('KeyMapDiv') == null)
    {
		...
		mapstyle.backgroundColor = '#808080';
		mapstyle.color = "#ff0000";
  ...
  mapdiv.innerHTML = '<table border="0" cellpadding="0" cellspacing="0" style="border:3px solid #0e88af;background-color:#599cf8;width:100%;"><tr>'
						+ '<td style="background-color:#0e88af;color:#ff0000;" nowrap="nowrap"><b>&nbsp;Keypad Map - '
						+lang.substring(0,1).toUpperCase() + lang.substring(1)+'</b></td><td bgcolor="#0e88af" nowrap="nowrap" width="20" align="right">'
						+ '<div align="right" onclick="hideMap()" style="padding:2px;width:20px;text-align:right;background-color:#0e88af;color:#ff0000;cursor:default">'
...
function showCombi(e)
{
	...
	{	helpstyle.display = 'block';	return;	}
	...
	if( prevWord != "" && !isLangOtru(prevWord) && prevLet != getLang('',Char,0) )
	{
		...
		{
		    helptxt = '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + ' + Char + ' = <b>' + getLang(prevWord, Char, 0) + "</b></td>";
			if(Char == 'a')
			    helptxt += '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + i = <b>' + getLang(prevWord, 'i', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
						  + prevWord + ' + u = <b>' + getLang(prevWord,'u',0) + "</b></td>";
		}
		else if( Char != getLang('',Char,0))
		{
		    helptxt = '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + a = <b>' + getLang(prevWord, 'a', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + A = <b>' + getLang(prevWord, 'A', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + i = <b>' + getLang(prevWord, 'i', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + I = <b>' + getLang(prevWord, 'I', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + u = <b>' + getLang(prevWord, 'u', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + U = <b>' + getLang(prevWord, 'U', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + e = <b>' + getLang(prevWord, 'e', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + E = <b>' + getLang(prevWord, 'E', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + a + i = <b>' + getLang(getLang(prevWord, 'a', 0), 'i', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + o = <b>' + getLang(prevWord, 'o', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + o = <b>' + getLang(prevWord, 'O', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
				+ prevWord + ' + a + u = <b>' + getLang(getLang(prevWord,'a',0),'u',0) + "</b></td>"
			if(lang == 'tamil')
			{
				if(getLang('','t',0) == prevWord)
				    helptxt += '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + h = <b>' + getLang(prevWord, 'h', 0) + "</b></td>";
				if(getLang('','s',0) == prevWord)
				    helptxt += '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + h = <b>' + getLang(prevWord, 'h', 0) + "</b></td>";
				if(getLang('','S',0) == prevWord)
				    helptxt += '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + r + I = <b>' + getLang(getLang(prevWord, 'r', 0), 'I', 0) + "</b></td>";
				if(getLang('k','n',0).indexOf(prevWord) > 0 )
				    helptxt += '<td style="font-size:12px;border:1px solid #d0d9d9;" onclick="assignpredictionvalue(this)">' + prevWord + ' + t + h = <b>' + getLang(getLang(prevWord, 't', 0), 'h', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
								+ prevWord + ' + g = <b>' + getLang(prevWord, 'g', 0) + "</b></td><td style='font-size:12px;border:1px solid #d0d9d9;' onclick='assignpredictionvalue(this)'>"
								+ prevWord + ' + j = <b>' + getLang(prevWord,'j',0) + "</b></td>";
				
			}
		}
	
		helpdiv.innerHTML = '<table cellpadding="2" id="myTable" cellspacing="0" border="0" style="border:1px solid #d0d9d9;background-color:#599cf8"><tr>' + helptxt + '</tr></table>';	
...
function getcurretntcontrol(controlid) {
    sessionStorage.currentid = controlid.id;
}
function assignpredictionvalue(value) {
    var innervalue = $(value).children().html();
    var targetid = sessionStorage.currentid;
    var removelast = $("#" + targetid).val().substring(0, $("#" + targetid).val().length - 2);
    $("#" + targetid).val(removelast + innervalue);
    $("#HelpDiv").hide();
}
...
function convertTamil(ctrl, Key, dest)
{
	Char = String.fromCharCode(Key);
	var numchar = 4;
	...
	else
	{
		...
		myField = document.getElementById(dest);
		//myField = dest;
		if( myField.selectionStart >= 0)
		{
			if(isSplKey(Key) )
...
function convertTamil1(eng_text)
{
	...
	for (var i = 0, len = eng_text.length; i < len; i++)
	{
		Char = eng_text.charAt(i);
		Key = eng_text.charCodeAt(i);

		if( isIE )
		{
			//IE9 processs the event faster. so cancel the bubble first and then process the key
			myField.caretPos = document.selection.createRange().duplicate();
			prevChar = myField.caretPos.value;
			diff = 0;
			cpos = getCursorPosition(myField);
			if(prevChar.length != 0)
				document.selection.clear();
			...
			myField.caretPos.value = getLang(prevChar,Char, 0)
		}
		else
		{
			if( myField.selectionStart >= 0)
